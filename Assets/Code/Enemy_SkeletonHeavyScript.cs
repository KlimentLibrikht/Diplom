...
public class Enemy_SkeletonHeavyScript : MonoBehaviour
{
    ...
    void Update()
    {
        if (_enemyController.isDead == false && _enemyController.isAwake == true)
        {
            ...
            _enemyController.EnemyAttack("SkeletonHeavyAttack");
        }
    ...
    IEnumerator Who_IsDead(float sec)
    {
        ...
        if (_enemyController._healthEnemy <= 0 && _enemyController.isDead == false)
        {
            ...
            _enemyController._animator.SetTrigger("SkeletonHeavyDead");
